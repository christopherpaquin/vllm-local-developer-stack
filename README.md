...
See `deploy/.env.example` for every available option and its explanation.
...
changing `deploy/.env` (via `tune-inference.sh`) or host tuning (via
...
`deploy/.env`. If `deploy/.env` doesn't exist yet, it's created from
...
Review `deploy/.env` before continuing — you may manually adjust any
value.
...
- `check-yaml` — validates YAML syntax (e.g. `deploy/docker-compose.yml`, `.pre-commit-config.yaml`)
...
| Model | Resolved live from `GET /v1/models` if the server is reachable (typically `qwen2.5-coder-32b-awq`, matching `--served-model-name`) — falls back to `MODEL=` from `deploy/.env` with a warning if it isn't (only relevant when run on the vLLM host itself, since that's the only place `deploy/.env` exists) |
